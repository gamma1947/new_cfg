...
def read_fasta(fasta_path):       #Reads a FASTA file and returns a list of sequences.
    sequences = []
    current_seq = []
...
def train_markov_and_score(fasta_path, m):
    ...
    tm_log = markov_model_numpy(counts, m, pseudocount)

    for seq in sequences:
        ll = log_likelihood_numpy(tm_log, seq, m, base_map)
        print(ll)


def main():
    parser = argparse.ArgumentParser(description="Simpler Markov Model Trainer")
